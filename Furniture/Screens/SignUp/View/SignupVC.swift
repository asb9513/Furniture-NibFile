...
class SignupVC: UIViewController {
    ...
    func subscribeToResponse() {
        loginViewModel.loginModelObservable.subscribe(onNext: {
            if $0.status == true {
                ToastManager.shared.showError(message: "Registeration process done successfully", view: self.view, backgroundColor: .green)
               let sampleVC = MainTabBarVC(nibName: "MainTabBarVC", bundle: nil)
                ...
                ToastManager.shared.showError(message: "Error:email may be used befor or un completed deails ", view: self.view, backgroundColor: .red)
            }
        }).disposed(by: disposeBag)
    }
